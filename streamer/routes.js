...
router.get('/api/movie/download/:hash/:imdb/:token', (req, res) => {
	...
	admin.auth().verifyIdToken(authToken).then(decoded => {
		console.log('token for user ' + decoded.sub);
		admin.auth().getUser(decoded.sub).then(user => {
			return torrent.checkClient(downloadHash)
		}).catch(err => {
			res.json({ request: 407, data: err });
		})
	}).catch(err => {
...
router.get('/api/movie/check/:hash/:token', (req, res) => {
	...
	admin.auth().verifyIdToken(authToken).then(decoded => {
		console.log('token for user ' + decoded.sub);
		admin.auth().getUser(decoded.sub).then(user => {
			return files.watchMovieCheck(dirpath + hash)
		}).catch(err => {
			res.json({ request: 407, data: err });
		})
	}).catch(err => {
...
router.get('/api/series/get/imdb/:imdb_code', (req, res) => {
	...
	admin.auth().verifyIdToken(authToken).then(decoded => {
		console.log('token for user ' + decoded.sub);
		admin.auth().getUser(decoded.sub).then(user => {
			return seriesTorrent.getImdb(imdb)
		}).catch(err => {
			res.json({ request: 407, data: err });
		})
	}).catch(err => {
...
router.get('/api/series/download/:series_hash/:filename/:imdbid/:season/:episode/:token', (req, res) => {
	...
	torrent.checkClient(downloadHash)
		...
		.then((resolve) => {
			console.log('subtitles successfully downloaded');
			const checklink = 'http://localhost:3000/api/series/check/' + downloadHash + '/' + token;
...
router.get('/api/series/check/:hash/:token', (req, res) => {
	...
	admin.auth().verifyIdToken(authToken).then(decoded => {
		...
		admin.auth().getUser(decoded.sub).then(user => {
			...
			return files.watchSeriesCheck(dirpath + hash)
		}).catch(err => {
			res.json({ request: 407, data: err });
		})
	}).catch(err => {
	...
	files.watchSeriesCheck(dirpath + hash)
		...
		.then(
			(resolve) => {
				...
				const watchLink = 'http://localhost:3000/api/series/watch/' + hash + '/' + token;
...
router.get('/api/series/watch/:hash/:token', (req, res) => {
	...
	admin.auth().verifyIdToken(authToken).then(decoded => {
		console.log('token for user ' + decoded.sub);
		admin.auth().getUser(decoded.sub).then(user => {
			return torrent.seriesFile(hash)
		}).catch(err => {
			res.json({ request: 407, data: err });
		})
	}).catch(err => {
...
router.get('/api/show/get/details/:id/:show/:slug/:token', (req, res) => {
	...
	if (!err) {
		...
		admin.auth().verifyIdToken(authToken).then(decoded => {
			console.log('token for user ' + decoded.sub);
			admin.auth().getUser(decoded.sub).then(user => {
				return seriesTorrent.getShowData(arr)
			}).catch(err => {
				res.json({ request: 407, data: err });
			})
		}).catch(err => {
